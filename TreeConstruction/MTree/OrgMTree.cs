...
namespace TreeConstruction
{
    public class OrgMTree
    {
        ...
        /// </summary>
        /// <returns></returns>
        public MTreeNode CreateFakeMTree()
        {
            //构建根节点
            MTreeNode root = new MTreeNode();
            root.Childrens = new List<MTreeNode>();

            #region 构建 Hashtable
            Dictionary<string, MTreeNode> dic = new Dictionary<string, MTreeNode>();
            foreach (OrgModel item in Orgs)
            {
                MTreeNode node = new MTreeNode();
                node.Childrens = new List<MTreeNode>();
